...
**P0-3a. Auto-numbered QRD headings (added 2026-09-09).** Many MAH-authored SmPCs carry the QRD section numbers as Word *automatic numbering* (`w:numPr` on the heading paragraph, resolved through `numbering.xml`) rather than as typed text. `mammoth` drops automatic numbering, so such headings reach the section splitter as `Name of the medicinal product` / `Therapeutic indications` and match nothing — the P0-2 gate then returns 422 on a perfectly valid SmPC. `read_docx` therefore runs a numbering pre-pass (`_materialise_qrd_numbering`) on the raw `document.xml` before mammoth: it resolves each numbered paragraph's label (direct or style-inherited `numPr`; `decimal`, `decimalZero`, letter and roman formats; `lvlText` templates such as `%1.` / `%1.%2`; `start` and `startOverride`; deeper levels reset when a shallower level advances) and, **only** when `label + " " + text` matches an `SMPC_HEADERS` / `PIL_HEADERS` pattern and the text does not already start with a number, writes the label into the paragraph as literal text and cancels its numbering in place (`numPr` → `ilvl 0 / numId 0`, so mammoth emits a heading/paragraph, not an `<ol><li>` the splitter would refuse, even for styles linked to a list level). A QRD-order guard adds a second condition: within a heading family the number must advance the sequence (SmPC: forward only; PIL: strict 1→6 succession) or restart at 1 — typed headings advance the state too — so a body list item that happens to read like a section, or a paragraph whose number drifted on re-processing, is never promoted. Every other paragraph — body lists, bullets, already-numbered headings, paragraphs inside text boxes — is left byte-for-byte untouched. Stdlib only (`zipfile` + string surgery on `document.xml`, depth-aware paragraph scanner, `mc:Fallback` duplicates masked, tracked changes honoured); deterministic; idempotent; O(n) in document size.

*Acceptance criteria:*
- Given `tests/fixtures/synthetic_smpc_autonum.docx` (the Synthex SmPC with every QRD heading auto-numbered and one auto-numbered body list in 4.2), when parsed, then the section IDs are identical to those of `synthetic_smpc.docx`, the body list keeps its `numPr` (renders as `<ol>`), and `POST /api/process_stateless` returns 200, not 422.
- Given `synthetic_smpc.docx` or `synthetic_smpc_images.docx` (typed numbers, no `numPr` on headings), then the pre-pass returns the input bytes unchanged.
- Given the pre-pass output fed to the pre-pass again, then the bytes are unchanged (idempotent).
